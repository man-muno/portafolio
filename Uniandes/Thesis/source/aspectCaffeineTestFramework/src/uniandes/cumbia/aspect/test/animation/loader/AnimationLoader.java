...
import uniandes.cumbia.aspect.test.animation.Animation;
...
public class AnimationLoader extends AbstractLoader
{
    ...
    private static final String SET_CONFLICT_FILE = "setConflictFile";
    
    private static final String DISABLE_ASPECT = "disableAspect";
    ...
    public void loadElement( ) throws LoaderException
    {
        ...
        for(int i=0;i<nodes.getLength( );i++)
        {
            ...
            else if(childNode.getNodeName( ).equals( ADD_ASPECT ))
            {
                AddAspectLoader loader = new AddAspectLoader(childNode);
                loader.loadElement( );
                animation.addAnimationCommand( loader.getElement() );
            }
            else if(childNode.getNodeName( ).equals( SET_CONFLICT_FILE ))
            {
                SetConflictFileLoader loader = new SetConflictFileLoader(childNode);
                loader.loadElement( );
                animation.addAnimationCommand( loader.getElement() );
            }
            else if(childNode.getNodeName( ).equals( DISABLE_ASPECT ))
            {
                DisableAspectLoader loader = new DisableAspectLoader(childNode);
                loader.loadElement( );
                animation.addAnimationCommand( loader.getElement() );
            }
            else if(childNode.getNodeName( ).equals( REMOVE_ASPECT ))
            {
                RemoveAspectLoader loader = new RemoveAspectLoader(childNode);
                loader.loadElement( );
                animation.addAnimationCommand( loader.getElement() );
            }
        }
