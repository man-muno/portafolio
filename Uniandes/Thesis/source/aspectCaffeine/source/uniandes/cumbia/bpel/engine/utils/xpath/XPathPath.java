 ...
 */
public abstract class XPathPath extends XPathExpression {

    private XPathStep[] steps;
    private boolean basicKnown = false;
    private boolean basic;

    public XPathPath(XPathStep[] steps) {
        this.steps = steps;
    }

    public XPathStep[] getSteps() {
        return steps;
    }
     ...
     */
    protected boolean isSimpleStep(XPathStep step) {
        if (step.getAxis() == Compiler.AXIS_SELF) {
            XPathNodeTest nodeTest = step.getNodeTest();
            if (!(nodeTest instanceof XPathNodeTypeTest)) {
                return false;
            }
            int nodeType = ((XPathNodeTypeTest) nodeTest).getNodeType();
            ...
            XPathNodeTest nodeTest = step.getNodeTest();
            if (!(nodeTest instanceof XPathNodeNameTest)){
                return false;
            }
            
            if (((XPathNodeNameTest) nodeTest).isWildcard()) {
                return false;
            }
            return areBasicPredicates(step.getPredicates());
        }
    ...
    protected boolean areBasicPredicates(XPathExpression predicates[]) {
        if (predicates != null && predicates.length != 0) {
            boolean firstIndex = true;
            for (int i = 0; i < predicates.length; i++) {
                if (predicates[i] instanceof XPathNameAttributeTest) {
                    if (((XPathNameAttributeTest) predicates[i])
     ...
     */
    protected Pointer getSingleNodePointerForSteps(/*ElementMemory context*/) {
        /*if (steps.length == 0) {
...
*/        return null; //TODO ARREGLAR
    ...
    {
        /*if (nodeTest instanceof XPathNodeNameTest) {
            QName qname = ((XPathNodeNameTest) nodeTest).getNodeName();
